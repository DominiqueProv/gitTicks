...
it('returns a 401 if the user doesnt provide an invalid title or price', async () => {
...
it('update the ticket if valid input', async () => {
  const cookie = global.signin();
  const response = await request(app)
    .post('/api/tickets')
    .set('Cookie', cookie)
  ...
  expect(ticketResponse.body.price).toEqual(22222);
});
